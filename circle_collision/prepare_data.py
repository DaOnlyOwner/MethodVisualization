...
try:
    from gather_words import CIRCLE_COLLISION_DIRECTORY
except ImportError:
    # I get an import error, so I need to do that
    print("Warning: Cannot import CIRCLE_COLLISION_DIRECTORY")
    CIRCLE_COLLISION_DIRECTORY = Path(".")

SEED = 1202991

def load_data_all_clusters(input_dir):
...
def load_data(input_dir, num_clusters=50):
...
def prepare_cluster_data_d3js_all_clusters(working_dir):
    ...
    for clusters in clusters_list:
        ...
        alt_dir = working_dir / 'graph_circle' / ("visualization_"+str(len(clusters)))
...
def prepare_cluster_data_d3js(working_dir, num_clusters=50):
    ...
    data = {"amountClusters": len(clusters), "nodes": nodes, "edges": edges}
    jsfile = f"const data = {js.dumps(data)};"
    alt_dir = working_dir / 'graph_circle'
    alt_dir.mkdir(parents=True,exist_ok=True)
    with open(alt_dir / 'prepared_data.js', "w") as f:
        f.write(jsfile)
    copy2(CIRCLE_COLLISION_DIRECTORY/'d3.min.js', alt_dir/"d3.min.js")
    copy2(CIRCLE_COLLISION_DIRECTORY/'visualize.html', alt_dir / "visualize.html")
    copy2(Path(CIRCLE_COLLISION_DIRECTORY/'d3js_License.txt'), alt_dir / "d3js_License.txt")
...
if __name__ == "__main__":
    prepare_cluster_data_d3js_all_clusters(Path("../example_input"))
